...
```mermaid
graph TD
    subgraph MAL [Mind-at-Large: Ontological Ground]
        W[W-Axis: Strategic Morphological Intent]
    end

    W -->|Top-Down Patterning| Org[Organism: Integrated Cognitive Agent]
    Org -->|Physiological Set-Points| Tissue[Tissue: Bioelectric Morphogenetic Fields]
    Tissue -->|Voltage/Gap Junction Signal| Cell[Cellular Collective: Decision-Making]
    Cell -->|Epigenetic State & Protein Activity| Gene[Genome: Resource Library]

    Gene -.->|Toolkit for Proteins| Cell
    Cell -.->|Emergent Pattern Formation| Tissue
    Tissue -.->|System-Level Stability| Org
    Org -.->|Feedback & Learning| MAL

    style MAL fill:#f1c40f,stroke:#f39c12
    style Gene fill:#95a5a6,stroke:#7f8c8d
```
...
```mermaid
graph LR
    Input[Environmental / Hormonal Signal] --> Mesh[Bioelectric Sensory Mesh<br>Gap Junction Network & Ion Channels]
    Mesh -->|Encodes| TM[Target Morphology Attractor<br>Stable Vmem Pattern]
    TM -->|Instructs via<br>Bioelectric Cues| Cells[Cell Collective Agency]
    
    Cells -->|Execute via<br>Proteomic & Transcriptomic Changes| Form[Physical Morphogenesis]
    Form -->|Sensory Feedback<br>Mechanical / Electrical| Mesh

    subgraph Levin_Logic [Levin: Cognitive Bioelectricity]
        Mesh
        TM
    end
...
```mermaid
graph TD
    %% Define styles for the groups using classDef
    classDef intentLayer fill:#f8f9fa,stroke:#333,stroke-width:2px
    classDef compilerLayer fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    classDef bioMeshLayer fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    classDef infraNode fill:#2ecc71,stroke:#27ae60,stroke-width:2px,color:#fff

    %% Intent Layer (Z-Axis)
    subgraph Intent_Layer [Z-Axis: Commonwealth Strategic Intent]
        User[Guild & Ecosystem Need<br>e.g., 'Filter this estuary'] -->|Define Function| Spec[Desired Morpho-Function<br>e.g., 'Branching, High-Surface-Area Filter']
    end

    %% Compiler
    subgraph Compiler [The Anatomical Compiler]
        Spec -->|Translate| Signal[Bioelectric Signal Pattern<br>e.g., Specific Vmem Map]
        Signal -->|Map to| Morph[Target Morphospace Attractor]
    end

    %% Biological Mesh (Cellular Intelligence)
    subgraph Biological_Mesh [Cellular Intelligence Substrate]
        Morph -->|Induce via<br>Optogenetics / Ionoceutics| Ions[Ion Channel / Gap Junction Modulation]
        Ions -->|Trigger| Col[Cellular Collective Agency<br>Plasticity & Problem-Solving]
        Col -->|Execute Program| Growth[Guided Morphogenesis]
    end

    %% Final Output and Feedback Loop
    Growth -->|Result| Infrastructure[Living Regenerative Infrastructure]
    Infrastructure -->|Performance & Ecosystem Feedback| Spec

    %% Apply the styles defined at the top
    class Intent_Layer intentLayer;
    class Compiler compilerLayer;
    class Biological_Mesh bioMeshLayer;
