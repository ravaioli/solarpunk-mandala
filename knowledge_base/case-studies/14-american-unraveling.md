...
```mermaid
flowchart TD
    ICE[“ICE<br>Core Enforcement Node”]

    subgraph Capital_Flows[Capital & Profit Rhizome]
        PP[Private Prison Corporations<br>CoreCivic, GEO Group]
        BP[Banking & Finance]
    end

    subgraph Enforcement_Expansion[Enforcement Expansion Rhizome]
        LLE[Local Law Enforcement<br>287g Program]
        CBP[Customs & Border Patrol]
        SURV[Surveillance Tech<br>e.g., Palantir ELITE]
    end

    subgraph Political_Structure[Political & Legal Insulation]
        FED[Federal Budget & Patronage]
        OVD[Erosion of Oversight]
        LGL[Qualified Immunity &<br>Broad Legal Leniency]
    end

    ICE -- “Contracts & Profits<br>($45B Detention Fund)” --> PP
    PP -- “Campaign Finance & Lobbying” --> FED
    FED -- “Massive Budget Allocation” --> ICE

    ICE -- “Delegates Authority, Training” --> LLE
    LLE -- “Force Multiplier, Data” --> ICE
    ICE -- “Data Fusion & Targeting” --> SURV
    SURV -- “Medicaid/Utility Data<br>‘Confidence Scores’” --> ICE

    OVD -- “Denies Congressional Access<br>Dismantles Ombudsman” --> ICE
    LGL -- “Protects from Accountability” --> ICE
    
    style ICE fill:#ffcccc,stroke:#333,stroke-width:3px
    style PP fill:#ffebcc,stroke:#333
    style FED fill:#e6ccff,stroke:#333
```
...
```mermaid
flowchart TD
    MA[Mutual Aid &<br>Direct Support]
    PP[Political &<br>Narrative Power]
    IP[Institutional Pressure<br>& Labor]

    LA[Legal Aid Collectives]
    HN[Housing & Sanctuary<br>Networks]
    MS[Material Support Funds]

    PA[Popular Assemblies]
    SC[Sanctuary City Policies]
    NC[Narrative Campaigns]

    LAB[Labor Unions &<br>Strike Organizing]
    DC[Divestment Campaigns]
    SL[Strategic Litigation]
    ...
    LAB --> DC
    DC --> PPv[Private Prisons]
    SL --> ICE[ICE Node]

    PPv -.-> ICE

    style MA fill:#d5f5e3,stroke:#333
    style PP fill:#e3f2fd,stroke:#333
    style IP fill:#fff3e0,stroke:#333
    style ICE fill:#ffcccc,stroke:#333,stroke-width:2px
    style PPv fill:#ffebcc,stroke:#333
```
...
```mermaid
flowchart TD
    Start["Present State<br>Game A Carceral Node"] --> P1
    
    subgraph P1[Phase 1: Crisis & Containment]
        direction LR
        P1_Obj[Objective<br>Stymie Expansion,<br>Provide Community Defense]
        P1_A1[Mobilize against<br>new camps]
        P1_A2[Expand Sanctuary<br>Networks]
        P1_A3[Universal<br>'Know Your Rights']
        P1_A4[Support<br>Whistleblowers]
    end

    P1 -- "Block activation of<br>~30,000 private prison beds" --> P2

    subgraph P2[Phase 2: Strategic Dismantling & Growth]
        direction LR
        P2_Obj[Objective<br>Sever ICE's Rhizome,<br>Build Alternative Capacity]
        
        subgraph Dismantling[Campaigns to Dismantle]
            D1[Defund & Divest]
            D2[Disrupt Partnerships]
        end

        subgraph Building[Initiatives to Build]
            B1[Scale Mutual Aid]
            B2[Formalize Welcome Centers]
        end
    end

    P2 -- "Dual power achieves<br>functional parity" --> P3

    subgraph P3[Phase 3: Transformation & Transition]
        direction LR
        P3_Obj[Objective<br>Legislate Abolition,<br>Implement New Systems]
        
        T1[Abolish ICE &<br>Terminate Contracts]
        T2[Reallocate to<br>Community Fund]
        T3[Implement<br>Just Transition]
    end

    P3 --> Future["Envisioned Future<br>Regenerative, Non-Carceral System<br>Community Welcome & Safety"]
    
    %% Styling
    style Start fill:#ffcccc,stroke:#333
    style Future fill:#ccffcc,stroke:#333
    style P1 fill:#ffe6cc,stroke:#333
    style P2 fill:#e6ccff,stroke:#333
    style P3 fill:#ccffff,stroke:#333
```
