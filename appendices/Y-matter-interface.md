...
```mermaid
flowchart TD
    subgraph Marxist_Frame[Historical Materialist Analysis]
        direction LR
        A[Mode of Production] --> B[Class Relations]
        B --> C[Ideological Superstructure]
        C --> D[Material Praxis]
    end

    Marxist_Frame -- TRANSLATION PROTOCOL --> Mandala_Frame

    subgraph Mandala_Frame[Mandala Geometric Analysis]
        direction LR
        E[LR Cube Pattern] --> F[Cube 5 Boundary]
        F --> G[UL/LL Cube Software]
        G --> H[Pathway Action]
    end
...
```mermaid
flowchart TD
    Start[Analyze a Social Institution] --> Step1
    subgraph Step1[Step 1: Identify the LR Pattern]
        S1A[Observe systemic rules] --> S1B[Name the core logic]
    end

    Step1 --> Step2
    subgraph Step2[Step 2: Map the Cube 5 Boundary]
        S2A[Who is in/out?] --> S2B[How is it policed?] --> S2C[What is its quality?]
    end

    Step2 --> Step3
    subgraph Step3[Step 3: Analyze UL/LL Software]
        S3A[Audit Justifying Narratives] --> S3B[Map Cultivated Emotions] --> S3C[Trace Identity Formation]
    end

    Step3 --> Step4
    subgraph Step4[Step 4: Locate Friction & Leverage]
        S4A[Find Cracks & Costs] --> S4B[Identify Latent Capacities] --> S4C[Plot Geometric Interventions]
    end

    Step4 --> Blueprint[Output: Conscious-Praxis Blueprint]
```
