...
```mermaid
graph LR
    subgraph Low_Dim [3D Strategy Space: X, Y, Z]
        PD[Prisoner's Dilemma] --> NE[Nash Equilibrium: Defect/Extract]
        NE --> TC[Tragedy of the Commons]
    end

    subgraph High_Dim [4D Strategy Space: X, Y, Z + W]
        W_Axis[Add W: Ontological Depth] --> DL[Dimensional Lifting]
        DL --> RA[Regenerative Attractor]
        RA --> SO[Symbiotic Optimum: Pareto Optimal]
    end

    Low_Dim -.->|Introduce W-Axis| High_Dim
    
    style High_Dim fill:#f9f9f9,stroke:#333,stroke-dasharray: 5 5
    style RA fill:#f1c40f,stroke:#f39c12
```
...
```mermaid
graph TD
    subgraph State_Vector [Agent State Vector S_i]
        X[X: Logic/Consistency]
        Y[Y: Intersubjective Coupling]
        Z[Z: Agentic Efficacy]
        W[W: Ontological Depth]
    end

    W -->|Increase| WeightShift[Shift in Weighting Functions]
    
    WeightShift --> U_ego["ω_x: Maximize Self (U_ego)"]
    WeightShift --> U_altru["ω_y: Maximize Others (U_altru)"]
    WeightShift --> U_total["ω_w: Maximize System (U_total)"]

    style U_total fill:#2ecc71,stroke:#27ae60,color:#fff
    style W fill:#3498db,stroke:#2980b9,color:#fff
```
...
```mermaid
flowchart LR
    Agent((Agent i)) -->|Action A_i| Interface{Arena Interface}
    Interface -->|Partial Derivative| PhiUpdate[Change in Systemic Φ]
    
    PhiUpdate -->|Reward Protocol| Reward[Reward ∝ ∂Φ / ∂A_i]
    Reward -->|State Update| AgentUpdate[Increase S_i: X, Y, Z, W]
    
    AgentUpdate -->|Re-entry| Agent
    
    subgraph MAL [Mind-at-Large Field]
        PhiUpdate
    end

    style MAL fill:#e8f4f8,stroke:#a2d2df
