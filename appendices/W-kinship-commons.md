...
```mermaid
flowchart TD
    A[Kinship Network Mapping<br>See the System] --> B[Care Economy Audit<br>Value the Work]
    B --> C[Intergenerational Boundary Medicine<br>Heal the Wounds]
    C --> D[Developmental Web Design Kit<br>Build New Structures]
    D --> E[Rites of Passage Protocols<br>Celebrate the Culture]
    E -.->|Feeds back into<br>a healthier system| A
```
...
```mermaid
flowchart TD
    subgraph S1 [Stage 1: Individual Reflection]
        A1[Facilitator gives prompt] --> A2[Members reflect privately<br>on care given/received]
    end

    subgraph S2 [Stage 2: Collective Weaving]
        B1[Place central anchor<br>e.g., Community Land] --> B2[Add nodes for<br>people & households]
        B2 --> B3[Draw yarn connections,<br>verbalizing each relationship]
    end

    subgraph S3 [Stage 3: Diagnosis & Insight]
        C1[Observe the emergent pattern] --> C2{Identify}
        C2 --> C3[Care Fractures<br>thin/sparse areas]
        C2 --> C4[Nodes of Resilience<br>thick clusters]
    end

    S1 --> S2 --> S3
    C3 --> D[Outcome: Reveals systemic<br>isolation & overburden]
    C4 --> D
```
...
```mermaid
sequenceDiagram
    actor M as Member Morgan
    actor C as Community Ledger
    actor R as Member Riley

    Note over M,R: Morgan needs childcare
    M->>C: Publishes a Commitment:<br>"Offer 2 hrs gardening"
    R->>C: Browses open Commitments
    R->>M: Accepts offer, provides childcare
    M->>R: Fulfills promise: 2 hrs gardening
    R->>C: Marks transaction complete
    Note over C: Trust & social capital<br>are recorded and strengthened
```
...
```mermaid
graph TD
    subgraph PodA[Kinship Pod A]
        A1[Household 1]
        A2[Household 2]
        A3[Household 3]
    end

    subgraph PodB[Kinship Pod B]
        B1[Household 4]
        B2[Household 5]
        B3[Household 6]
    end

    %% Connections within pods (strong ties)
    A1 <--> A2
    A2 <--> A3
    A3 <--> A1
    ...
    %% Connections between pods (weak ties / webbing)
    A2 -.-> B1
    A3 -.-> B3
    B2 -.-> A1

    PodA --> W[Larger Community Web]
    PodB --> W
```
...
```mermaid
graph TD
    subgraph W [Appendix W: Kinship Commons]
        direction LR
        W1[Kinship Pods]
        W2[Care Audit]
    end
    ...
    S[Appendix S<br>Symbiotic Commonwealth] -->|Provides<br>micro-governance model| W1
    T[Appendix T<br>Regenerative Economy] -->|Values & circulates| W2
    P[Appendix P<br>Consciousness-Based Education] -->|Embeds learning in| C[Community Rites]

    W -->|Heals boundaries for| M[Appendix M<br>Psychopathology]
    W -->|Enacts rituals from| B[Appendix B<br>Ritual Technology]
```
