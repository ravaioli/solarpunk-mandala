...
| :--- | :--- | :--- | :--- |
| **1. Local Assembly**<br>(Communalism) | Hyper-local, face-to-face governance and community building. | **Relational Depth (Y-Axis)** | Fosters intersubjectivity, empathy, and direct democratic participation. |
| **2. Functional Guild**<br>(Syndicalism) | Self-managed, productive labor organized by craft and purpose. | **Efficacy (Z-Axis)** | Channels skillful action and purposeful work into regenerative outcomes. |
| **3. Confederal Mesh**<br>(Confederalism) | Regional coordination through mandated, recallable delegates. | **Ontological (W-Axis)** | Serves the health, sovereignty, and integration of the whole social body. |
| **4. The Φ-Grid**<br>(Cyber-Socialism) | Real-time monitoring and logistic feedback via sensor networks & data. | **Logistical (X-Axis)** | Enables the flow of information and resources to maintain systemic vitality. |
...
```mermaid
graph TB
    subgraph Base [Hyper-Local Scale]
        A[Neighborhood Assembly] -->|Direct Democracy| B[Local Needs & Decisions]
    end

    subgraph Production [Productive Scale]
        B -->|Informs| C[Functional Guilds]
        C -->|Execute| D[Regenerative Work<br>Food, Energy, Care, Build]
    end

    subgraph Coordination [Regional Scale]
        A -->|Sends Recallable Delegate| E[Confederal Council]
        E -->|Coordinates| F[Bioregional Projects]
    end

    subgraph Nervous_System [System-Wide Scale]
        G[Φ-Grid: Sensor Network] -->|Monitors| H[Vital Signs & Φ-Scores]
        H -->|Publishes Urgency Signals| I[Guilds & Assemblies]
    end

    D -->|Impacts| G
    F -->|Impacts| G
    I -->|Guides Resource Allocation| C
    I -->|Informs Priorities| A

    style Base fill:#e8f5e9,stroke:#2e7d32
    style Production fill:#e3f2fd,stroke:#1565c0
    style Coordination fill:#f3e5f5,stroke:#7b1fa2
    style Nervous_System fill:#fff3e0,stroke:#ef6c00
```
...
```mermaid
graph LR
    S[Sensors & Community Audit] -->|Raw Data| O[Φ-Grid Oracle]
    O -->|Calculates| P[Φ-Score & Vitality Status]
    P -->|Publishes| D[Dashboard & Alert]
    
    subgraph Response [Autonomous Response Layer]
        D -->|"Urgency Signal (e.g., 'Soil Health Low')"| G[Functional Guilds]
        G -->|Self-Organize into Pod| A[Regenerative Action]
    end

    A -->|Changes State| E[Ecosystem / Social Field]
    E -->|New Data| S
