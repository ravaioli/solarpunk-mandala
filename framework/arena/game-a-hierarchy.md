...
flowchart TD
    ...
    NS -- Lobbying & Campaign Finance --> TNC
    NS -- Regulatory Capture --> TNC
    TNC -- Tax Avoidance & Capital Flight --> OF
    OF -- Anonymous Wealth & Secrecy --> EL
    EL -- Revolving Door & Placement --> NS
    EL -- Revolving Door & Placement --> MIC
    MIC -- Defense Contracts & Budgets --> TNC
    TNC -- Advertising & Narrative Shaping --> MED
    MED -- Consent Manufacturing & Legitimacy --> NS
    NS -- Government Contracts & Funding --> MIC
    MIC -- Threat Inflation & Secrecy --> MED
    MED -- Ownership & Ideological Control --> EL
    OF -- Legal/Financial Services --> TNC
    TNC -- Funding & Sponsorship --> EL
```
...
```mermaid
flowchart LR
    A[Government<br>or Corporate Donor] -- Financial Grant --> B(Think Tank)
    B -- Shapes --> C[Research Agenda & Framing]
    C -- Produces --> D["&quot;Independent&quot;<br>Policy Report"]
    D -- Enabled by --> E[Media Amplification]
    E -- Leads to --> F[Policy Adoption / Influence]
    F -- Generates --> G[Benefit to Donor]
    G -- Incentivizes --> A
```
...
```mermaid
sequenceDiagram
    participant G as Gulf Capital<br>(SWF / Family Office)
    participant D as Dubai DIFC<br>Manager / GP
    participant C as Cayman Islands<br>Fund / SPV
    participant W as Global Asset<br>(Real Estate, Tech, Securities)

    Note over G,D: Primary Flow of Capital
    G->>D: Capital Investment
    D->>C: Allocation to Vehicle
    C->>W: Acquisition / Investment
    W-->>C: Returns & Profits
    C-->>D: Fees & Distributions
    D-->>G: Returns to Investor

    Note over D,C: Parallel Flow of Legal Control
    D->>C: Strategic Direction
